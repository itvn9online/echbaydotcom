...
//			$c_link = web_link . '?' . $link_for_taxonomy . 'cat=' . $v->term_id;
		...
		//
		$str .= ',{id:' . $v->term_id . ',ten:"' . _eb_str_block_fix_content ( $v->name ) . '",lnk:"' . $c_link . '",order:' . $cat_order . ',avt:"' . _eb_get_cat_object( $v->term_id, '_eb_category_avt' ) . '",icon:"' . _eb_get_cat_object( $v->term_id, '_eb_category_favicon' ) . '",arr:[' . _eb_get_full_category_v2 ( $v->term_id, $taxx, $get_full_link ) . ']}';
	}
	$str = substr ( $str, 1 );
