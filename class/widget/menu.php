...
class ___echbay_widget_get_menu extends WP_Widget {
    ...
    function form( $instance ) {
        ...
        $added_menu = [];
        foreach ( $a as $k => $v ) {
            $added_menu[ $v->term_id ] = $v->name;
        }

        echo '<div>';
        __eb_widget_load_select( array( '' => '[ Chọn menu đã thêm (ưu tiên) ]' ) + $added_menu, $this->get_field_name( 'menu_id' ), $menu_id );
        echo '</div><br>';


        //
        __eb_widget_load_select( array( '' => '[ Chọn menu theo key ]' ) + $arr_to_add_menu, $this->get_field_name( 'menu' ), $menu );
    ...
    function widget( $args, $instance ) {
        ...
        if ( $menu_id != '' && $menu_id > 0 ) {
            echo '<div data-id="' . $menu_id . '" class="each-to-edit-menu"></div>';
