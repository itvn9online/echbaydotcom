...
function echbay_create_admin_menu() {
	...
	*/
	...
	/*
	* submenu -> Super Admin, Administrator, Editor
	*/
	add_submenu_page( $parent_slug, 'Danh sách đơn hàng', 'Đơn hàng', 'publish_pages', 'eb-order', 'func_include_eb_private_code' );
...
//	add_submenu_page( $parent_slug, 'Dọn dẹp các dữ liệu tạm trong quá trình sử dụng website', 'Xóa bộ nhớ tạm', 'administrator', 'eb-cleanup-cache', 'func_include_eb_private_code' );
	...
	*/
	add_submenu_page( $parent_slug, 'Phiên bản cao cấp, hỗ trợ nhiều tính năng hơn', 'Nâng cấp bản ', 'manage_options', 'eb-licenses', 'func_include_eb_private_code' );
...
//	$last_update_js = date_time;
	...
	$current_github_version = _eb_get_static_html ( $strCacheFilter, '', '', 2 * 3600 );
...
//		$current_github_version = _eb_getUrlContent( 'https://raw.githubusercontent.com/itvn9online/echbaydotcom/master/VERSION' );
	...
	$current_theme_github_version = _eb_get_static_html ( $strCacheFilter, '', '', 2 * 3600 );
