...
if (arr_for_save_domain_config != "") {
	arr_for_save_domain_config = jQuery.parseJSON(
		unescape(arr_for_save_domain_config)
	);

	for (let x in arr_for_save_domain_config) {
...
function get_leech_data_post_id(str, vitri) {
	...
	let b = jQuery("#id_post_begin").val() || "",
			...
			if (b[i] != "") {
				if (b[i].length == 1) {
					...
					// nếu có điểm kết thúc -> lấy mảng ngay trước vị trí kết thúc
						...
						// v2
						let arr_e = e.split("||");
						for (let ie = 0; ie < arr_e.length; ie++) {
...
function leech_data_format_price(f) {
	let format_price = jQuery("#details_format_price").val() || "";
	if (f.t_giacu.value != "") {
		if (format_price == "") {
			f.t_giacu.value = g_func.number_only(f.t_giacu.value);
		} else {
			f.t_giacu.value = f.t_giacu.value
				.toString()
				.replace(eval(format_price), "");
		}
	...
	if (f.t_giamoi.value != "") {
		if (format_price == "") {
			...
			f.t_giamoi.value = f.t_giamoi.value
				.toString()
				.replace(eval(format_price), "");
		}

		// nếu có giá thấp nhất -> so sánh xong mới lấy
		let min_price = jQuery("#details_min_price").val() || "";
		if (
			min_price != "" &&
			parseInt(f.t_giamoi.value.split(".")[0], 10) < parseInt(min_price, 10)
		) {
			ket_thuc_lay_du_lieu(
				0,
				'<span class="orgcolor cur" onclick="func_leech_data_lay_chi_tiet(\'' +
					current_url +
					"');\">Min price</span>"
			);
...
function function_rieng_theo_domain() {
	...
	if (f.t_img.value == "") {
		f.t_img.value =
			jQuery("#details_finish_url li:last a").attr("data-img") || "";
...
function add_and_edit_time_get_post() {
	...
	limit_time_for_reload_this_page =
		limit_time_for_reload_this_page - (0 - gian_cach_submit);
...
function func_download_img_content_to_my_host() {
	...
	if (jQuery("#leech_data_fix_content img.download-img-to-here").length == 0) {
		...
		let add_content = jQuery("#leech_data_fix_content").html() || "";
		document.frm_leech_data.t_noidung.value = add_content.replace(
			/\sdownload-src=/gi,
			" src="
		);
	...
	let file_name = get_img_filename(img);
...
function func_download_img_to_my_host(img, dm, file_name) {
	...
	if (img.split("/" + dm + "/").length == 1) {
		...
		ajaxl(download_url, "oi_download_img_to_my_host", 1, function () {
			let a = jQuery("#oi_download_img_to_my_host").html();
			a = a.replace(/\s/gi, "%20");
			console.log(a);
	...
	jQuery
		...
		.done(function (msg) {
			try {
				...
				ket_thuc_lay_du_lieu(
					0,
					'<span class="redcolor cur" onclick="func_leech_data_lay_chi_tiet(\'' +
						current_url +
						"');\">ERROR</span>"
				);
...
//		msg = msg.replace(/<link[^>]*>/gi, " ");
		...
		*/
			...
			//
			// let old_bay = jQuery("base").attr("href");
			...
			//
			let remove_html = $("#remove_html_before_leech").val() || "";
			if (remove_html != "") {
				console.log("Remove HTML: " + remove_html);
				remove_html = remove_html.split(",");
				for (let i = 0; i < remove_html.length; i++) {
					remove_html[i] = jQuery.trim(remove_html[i]);
					jQuery("#" + id + " " + remove_html[i]).remove();
				}
			}
...
function func_leech_data_lay_chi_tiet(push_url) {
	...
	let a = jQuery("#details_list_url li:first a").attr("href") || "",
	...
	if (a != "") {
		...
		if (
			dog("post_id_is_numberic").checked == true &&
		...
		// lấy xong xóa
		jQuery("#details_finish_url").append(
			"<li>" + jQuery("#details_list_url li:first").html() + "</li>"
		);
		...
		if (dog("bai_viet_nay_duoc_lay_theo_id").checked == true) {
			...
			if (f.t_id.value == "" || f.t_id.value == "0" || f.t_id.value == 0) {
				ket_thuc_lay_du_lieu(
					0,
					'<span class="redcolor cur" onclick="func_leech_data_lay_chi_tiet(\'' +
						current_url +
			...
			if (
				before_post_id_for_leech != "" &&
				f.t_id.value == before_post_id_for_leech
			) {
				ket_thuc_lay_du_lieu(
					0,
					'<span class="redcolor cur" onclick="func_leech_data_lay_chi_tiet(\'' +
		...
		f.t_sku_leech_data.value =
			a.split("//")[1].split("/")[0] + "-" + leech_data_sku;
		...
		leech_data_content(
			...
			function () {
				...
				for (let x in arr) {
					...
					if (arr[x].get != "") {
						...
						if (typeof arr[x].youtube != "undefined") {
							...
							a =
								jQuery(arr[x].get).attr("data-old-src") ||
						...
						// nếu là hình ảnh
							...
							if (arr_get_img.split("[").length > 1) {
								...
								for (let z = 0; z < arr_get_img.length; z++) {
									let get_attr = arr_get_img[z].split("[");
									...
									if (get_attr.length > 1) {
										...
										if (str == "") {
											str =
												jQuery(get_attr[0]).attr(get_attr[1].split("]")[0]) ||
												"";
										}
									} else {
										if (str == "") {
						...
						// mặc định là lấy chữ
							...
							let a2 = arr[x].get.replace(/\s?\+\+\s?/g, "&&").split("&&");
								...
								if (a2[i].split(":each").length > 1) {
									...
									for (let j = 0; j < str_query.length; j++) {
										if (str_query[j].split(":each").length > 1) {
											...
											// bắt đầu vòng lặp
											jQuery(str_query[j].replace(/\s?\|\|\s?/g, ",")).each(
												function () {
													let get_html = "";
													if (typeof arr[x].strip_tags != "undefined") {
														get_html = jQuery(this).text() || "";
													} else {
														get_html = jQuery(this).html() || "";
													}
					...
					//
					jQuery(
						"form[name='frm_leech_data'] input[name='" +
							arr[x].set +
							"'], form[name='frm_leech_data'] textarea[name='" +
							arr[x].set +
							"']"
					).val(a);
				}
				...
				//
				let min_title = jQuery("#min_title_length").val() || "";
				if (min_title == "") {
				...
				// xử lý phần danh mục
				let check_cat1 = jQuery.trim(jQuery("#details_category").val() || "");
				if (check_cat1 != "") {
				...
				if (dog("get_last_url_segments").checked == true) {
					let t_seo = jQuery.trim(f.t_source.value);
					t_seo = t_seo.split("/");
					if (t_seo[t_seo.length - 1] != "") {
						t_seo = t_seo[t_seo.length - 1];
					} else {
						t_seo = t_seo[t_seo.length - 2];
					}
					if (dog("split_last_html").checked == true) {
						let str_post_end = jQuery("#id_post_end").val() || "";
						if (str_post_end != "") {
							t_seo = t_seo.split(str_post_end)[0];
						}
						t_seo = t_seo.split(".");
						if (t_seo.length > 1) {
							t_seo[t_seo.length - 1] = "";
							t_seo = t_seo.join(".");
						}
						f.t_seo.value = t_seo;
					}
				...
				//
				f.t_new_category.value = jQuery.trim(
					g_func.strip_tags(f.t_new_category.value)
				);
				f.t_new_2category.value = jQuery.trim(
				...
				f.t_noidung.value = f.t_noidung.value
					.replace(/\&nbsp\;/gi, " ")
					.replace(/\s+\s/g, " ");
...
//					console.log(a);
			...
			*/
				...
				/*
            if ( img_tags != '' ) {
            	let a = jQuery(img_tags).attr('data-old-src') || jQuery(img_tags).attr('data-src') || '';
            ...
            */
				...
				//
				if (
					dog("download_img_to_my_host").checked == false &&
...
function check_lech_data_submit(_alert) {
	...
	// ID phân nhóm
	f.t_ant.value = g_func.number_only(
		jQuery('#oiAnt input[name="t_ant"]').val() || 0
	);
...
function ket_thuc_lay_du_lieu(id, m, lnk) {
	if (typeof id == "number" && id > 0) {
		...
		jQuery("#details_finish_url li:last").append(
			' - <a href="' + lnk + '" target="_blank">' + m + "</a>"
		);
	} else {
...
jQuery("#categories_url")
	...
	.change(function () {
				...
				if (check_category_by_auto_slug(a[i]) != false) {
					...
					if (a[i].split("|").length > 2) {
						...
						a[i][4] =
							...
							(jQuery('#oiAnt input[name="t_ant"]').val() || 0) +
							"&caregory_source=" +
							encodeURIComponent(a[i][0]) +
							'" target="target_eb_iframe">[Tạo nhóm]</a>';
		...
		if (str_slug_error != "") {
			//		if ( jQuery.trim( jQuery('#details_category').val() || '' ) == '' ) {
			str_slug_error =
				'<li class="redcolor">URL có slug auto nhưng bị lỗi không tìm thấy</li>' +
				str_slug_error;
		...
		if (a == "") {
			...
			if (b != "") {
				...
				jQuery('#oiAnt input[type="text"]').val(
					b.split(".htm")[0].split(".asp")[0].split(".")[0]
				);
			}
		}
...
function EBE_save_cookie_to_data_base() {
	...
	let a = [];
	for (let i = 0; i < arr_save_checkbox_options.length; i++) {
		if (dog(arr_save_checkbox_options[i]).checked == true) {
			a.push(arr_save_checkbox_options[i]);
		}
	}
	arr_for_save_domain_config[EBE_current_first_domain][
		"save_checkbox_options"
	] = a;
	console.log(arr_for_save_domain_config);

	//
	document.frm_leech_data_save.t_noidung.value = escape(
		JSON.stringify(arr_for_save_domain_config)
	);
...
function EBE_auto_save_domain_cookie() {
    ...
    if ( dog('leech_data_auto_next').checked == true ) {
    	...
    	*/
	let f = document.frm_leech_data_save;
...
function create_list_post_for_crawl(a, img) {
	...
	if (dog("bai_viet_nay_duoc_lay_theo_id").checked == true) {
		let get_id = get_leech_data_post_id(a);
...
function after_list_post_for_crawl(str) {
	...
	//
	if (dog("leech_data_auto_next").checked == true) {
		let check_ant_select = g_func.number_only(
			jQuery('#oiAnt input[name="t_ant"]').val() || 0
		);
...
function WGR_leech_data_after_load_iframe() {
	setTimeout(function () {
		...
		jQuery("#crawl_eb_iframe")
			.contents()
			.find(current_loading_tags)
			.each(function () {
				let a =
...
jQuery(".click-submit-url-details")
	.off("click")
	.click(function () {
		let a = check_lech_data_submit("no");
        ...
        if ( a == 2 ) {
			...
			if (
				confirm("Chưa chọn Phân nhóm sản phẩm! Bạn có muốn chọn lại không?") ==
				true
			) {
				return false;
			}
...
jQuery("#oi_save_list_category")
	...
	.click(function () {
		...
		let min_height = jQuery(this).attr("data-min-height") || 60,
			add_height = jQuery(this).attr("data-add-height") || 20;

		let new_height = jQuery(this).get(0).scrollHeight || 0;
...
jQuery(".add-db-list-post-to-process")
	.off("click")
	.click(function () {
		let a = jQuery("#oi_save_list_category").val() || "";
		if (a != "") {
		...
		if (
			...
			setTimeout(function () {
				...
				setTimeout(function () {
					...
					jQuery("#show_next_page_leech").html(
						cache_node_for_auto_leech +
							"/ " +
							(jQuery("#oi_save_list_category").val().split("\n").length - 1)
					);

					// tăng cache lên 1 đơn vị rồi lưu lại -> lần nạp trang tới sẽ sử dụng cái này
					cache_node_for_auto_leech = cache_node_for_auto_leech - (0 - 1);
					leech_data_save_cookie(
...
function add_parameter_for_auto_leech(start) {
	let u = window.location.href.split("&auto_leech=1")[0];
...
setTimeout(function () {
	...
	if (check_auto_leech_on_off()) {
		...
		jQuery("#star_get_random_category_for_leech").html(
			'<i class="fa fa-pause"></i> ' +
				jQuery("#star_get_random_category_for_leech").attr("data-stop")
		);
	...
	// nếu không, chỉ tìm cache và xử lý bình thường
		...
		(function () {
			//let a = g_func.getc("ck_old_categories_url");
			let a = ld_local_get_item("ck_old_categories_url");
	...
	jQuery("#star_get_random_category_for_leech")
		.attr({
			//		'data-stop': jQuery('#star_get_random_category_for_leech').attr('data-stop') || '',
			"data-start":
				jQuery("#star_get_random_category_for_leech").attr("data-start") ||
				jQuery("#star_get_random_category_for_leech").html() ||
				"",
		})
		.off("click")
		.click(function () {
			...
			if (check_auto_leech_on_off()) {
				...
				jQuery(this).html(
					'<i class="fa fa-play"></i> ' + jQuery(this).attr("data-start")
				);
			}
			// chạy tiếp
			else {
				...
				jQuery(this).html(
					'<i class="fa fa-pause"></i> ' + jQuery(this).attr("data-stop")
				);
			}
...
setInterval(function () {
	...
	if (check_auto_leech_on_off()) {
		...
		if (
			tu_dong_load_lai_trang_neu_submit_loi > limit_time_for_reload_this_page
		) {
			setTimeout(function () {
				console.log(5);
			}, 1000);
		...
		// nếu không cứ 10 giây thêm 1 đơn vị
			...
			console.log(
				"Reload page after " +
					...
					"s. Next after " +
					(gian_cach_submit - tu_dong_load_lai_trang_neu_submit_loi) +
					"s"
			);
		}
	}
...
jQuery(".click-submit-url-categories")
	.off("click")
	.click(function () {
		//
		let html_tags = jQuery("#categories_tags").val() || "";
		...
		//
		let a = jQuery("#categories_list_url li:first").html() || "";
		a = g_func.trim(a);
		...
		if (a != "") {
            ...
            */
			...
			if (jQuery("#categories_url").val() != "") {
				...
				jQuery("#show_text_after_done").append(
					"<li>Không tìm thấy danh sách nhóm cần lấy sản phẩm</li>"
				);
				window.scroll(0, jQuery("#show_text_after_done").offset().top - 90);

				// nạp lại trang sau khi hoàn thành
				if (
					dog("nap_lai_trang_sau_khi_hoan_thanh").checked == true ||
...
(function (arr) {
	...
	if (a != null && typeof a == "string") {
		jQuery('select[name="post_tai"]').val(a) ||
			...
			jQuery('select[name="post_tai"] option[value="' + a + '"]').attr(
				"selected",
				"selected"
			);
	}
...
(function () {
	...
	if (arr_cookie_lamviec == null) {
		...
		for (let x in default_arr_cookie_lamviec) {
	...
	for (let x in arr_cookie_lamviec) {
		if (x == "save_checkbox_options") {
			let arr_check_op = arr_cookie_lamviec[x];

			for (let j = 0; j < arr_check_op.length; j++) {
...
jQuery(
	'#oiBlogAnt option[data-show="1"], #oiBlogAnt option[value="0"]'
).remove();
