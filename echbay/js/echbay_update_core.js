...
function update_wgr_plugin_theme_done(h, done_or_error) {
	...
	if (done_or_error > 0) {
		...
		show_done1 =
			"Lỗi trong quá trình cập nhật, vui lòng thử lại sau ít phút...";
	...
	timeout_remove_update_cache_file = setTimeout(function () {
		...
		jQuery("body")
			.removeClass("wgr-waiting-update-complete")
		...
		window.open(
			window.location.href
				.split("&confirm_eb_process=")[0]
				.split("&remove_update_running_file=")[0] +
				"&remove_update_running_file=1",
			"target_eb_iframe"
		);
...
function update_wgr_plugin_theme_begin(i) {
	...
	jQuery("body")
		.addClass("wgr-waiting-update-complete")
		.addClass("ebdesign-no-scroll");
...
function update_wgr_plugin_theme_alert() {
	a_lert(
		"Quá trình Cập nhật này vừa được thực thi, không thể thực hiện lại ngay được!"
	);
}

// cập nhật plugin từ server của EchBay.com
jQuery(".click-connect-to-echbay-update-eb-core")
	.attr({
		target: "target_eb_iframe",
		href:
			window.location.href.split("&confirm_eb_process=")[0] +
			"&confirm_eb_process=1",
	})
	.click(function () {
		//
...
jQuery(".click-connect-to-github-update-eb-core")
	.attr({
		target: "target_eb_iframe",
		href:
			window.location.href.split("&confirm_eb_process=")[0] +
			"&confirm_eb_process=1&connect_to=github",
	})
	.click(function () {
		//
		//	alert( document.domain );
...
// cập nhật theme
jQuery(".click-connect-to-echbay-update-eb-theme")
	.attr({
		target: "target_eb_iframe",
		href:
			window.location.href.split("&confirm_eb_process=")[0] +
			"&confirm_eb_process=1&connect_to=theme",
	})
	.click(function () {
		//
		//	alert( document.domain );
...
//
if (window.location.href.includes("&confirm_eb_process=") == true) {
	// nếu lệnh đang chạy trong iframe -> lệnh update đang chạy -> hiển thị thông báo đang update
	if (top != self) {
		parent.update_wgr_plugin_theme_done(
			jQuery("#get_list_wgr_process_update").html(),
			jQuery("#eb_core_update_all_done").length || 0
		);
	}
...
// tự chạy chức năng update khi click từ link sang
else if (window.location.href.includes("&click_to_update_core=") == true) {
	...
	if (top == self) {
		setTimeout(function () {
			...
			// update theme
			if (window.location.href.includes("&connect_to=theme") == true) {
				a =
					jQuery(".click-connect-to-echbay-update-eb-theme").attr("href") || "";
			...
			// update plugin
				...
				a =
					jQuery(".click-connect-to-github-update-eb-core").attr("href") || "";
