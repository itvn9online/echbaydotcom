	...
	*/
	...
	for (let i = 0; i < arr.length; i++) {
		...
		if (arr[i] != "") {
			...
			if (
				arr[i].substr(0, 4) == "<!--" &&
				arr[i].substr(arr[i].length - 3) == "-->"
			) {
			} else if (
				arr[i].substr(0, 2) == "/*" &&
				arr[i].substr(arr[i].length - 2) == "*/"
			) {
			}
			// accept code
...
function check_update_config() {
	...
	if ( $.trim( f.cf_dns_prefetch.value ) != '' ) {
		let a = f.cf_dns_prefetch.value.split('//');
//		console.log( a.length );
	...
	*/
	...
	//
	let a = $.trim(f.cf_current_price.value);
	if (a != "") {
...
function create_deault_css() {
	...
	if (
		$("#mcb2__custom_css_in_js__color").length > 0 &&
		f.mcb2__custom_css_in_js__color.value != ""
	) {
		root_color.push(
			"--default2-color: " + f.mcb2__custom_css_in_js__color.value
		);
	...
	if (a == "full") {
		...
		str +=
			"@media screen and (max-width:775px) { .oi_big_banner {height: auto;line-height: normal;} }";
		str +=
			"@media screen and (max-width:350px) { .oi_big_banner {height: auto;line-height: normal;} }";
	}
	// những cái khác định dạng bình thường
		...
		a = Math.ceil(a / 2);
		str +=
			"@media screen and (max-width:775px) { .oi_big_banner {height: " +
			a +
			"px;line-height: " +
			a +
			"px;} }";
		...
		a = Math.ceil(a / 2);
		str +=
			"@media screen and (max-width:350px) { .oi_big_banner {height: " +
			a +
			"px;line-height: " +
			a +
			"px;} }";
	...
	*/
	...
	str +=
		".mcb .thread-home-c2, .mcb .thread-home-c2 a, .mcb .echbay-widget-title, .mcb .echbay-widget-title a { color: " +
		a +
		" !important; }";
	...
	str +=
		".default-bg, .thread-home-c2 a:first-child, #oi_scroll_top { background-color: " +
		a +
		"; }";
	str +=
		".div-search, .thread-home-c2, .echbay-widget-title:before, .echbay-widget-title div, .default-border-color { border-color: " +
		a +
		"; }";
	...
	str +=
		".default-2bg, .default2-bg, #oi_scroll_top:hover { background-color: " +
		a +
		"; }";
	...
	str += create_css_for_custom_in_js("", "", {
		for: "logo",
		mobile:
			".style-for-mobile #webgiare__top .web-logo, .style-for-mobile .webgiare__top .web-logo, .style-for-mobile .menu-for-mobile .web-mobile-logo",
		pc: ".web-logo",
	});
	...
	str += create_css_for_custom_in_js("", "", {
		for: "footer_logo",
		mobile: null,
		pc: ".web2-logo .web-logo, #webgiare__footer .web-logo, footer#footer .web-logo, #footer-lazyload .web-logo",
	});
	...
	*/
	...
	try {
		...
		if (a != "" && a.substr(0, 1) == "#" && (a.length == 4 || a.length == 7)) {
			str +=
				".thread-details-tab,.thread-list-filter{border-bottom-color:" +
				a +
				"}.thread-list-filter div{background-color:" +
				a +
				"}";
		}
	} catch (e) {
...
function create_phone_click_to_call() {
	...
	// điện thoại
	let a = $.trim(f.cf_dienthoai.value),
		str = "";
	if (a != "") {
		a = a.split("\n");
		...
		for (let i = 0; i < a.length; i++) {
			...
			if (a[i] != "") {
				...
				str +=
					'<a href="tel:' +
					$.trim(a[i].toString().replace(/[^0-9\+]/g, "")) +
					'" rel="nofollow">' +
					a[i] +
					"</a>" +
					"\n";
...
function config_test_send_mail() {
	...
	ajaxl(web_link + "test_email?email=" + to, "test_smtp_email", 1, function () {
		$("#test_smtp_email").html(
			$("#test_smtp_email").html().replace(/\n/g, "<br>")
		);
	});
...
if (current_module_config != "config_theme") {
	...
	(function () {
		...
		load_config_for_custom_logo(
			...
			{
				background_position:
					"Căn vị trí hình ảnh sẽ xuất hiện trên khung của logo",
				background_size:
					"Với mỗi logo, sẽ có tỷ lệ giữa chiều rộng với chiều cao khác nhau, dùng thuộc tính này để định hành tránh bị vỡ khung logo",
				background_position_for_mobile:
					"Sử dụng khi bạn muốn logo trên bản mobile ở một vị trí khác so với bản desktop",
				background_size_for_mobile:
					"Sử dụng khi bạn muốn logo trên bản mobile có kích thước khác so với bản desktop",
			},
			{},
			//			a = $('#cf_css_logo').val() || '',
			data["cf_css_logo"],
		...
		load_config_for_custom_logo(
			...
			{
				background_image:
					"Sử dụng khi bạn muốn logo trên bản mobile khác với Logo bản desktop. Copy và nhập URL ảnh vào đây là được, tính năng này chỉ hỗ trợ các phiên bản theme có sử dụng logo trên mobile.",
			},
			{},
			data["cf_css_mobile_logo"],
			{
		...
		load_config_for_custom_logo(
			...
			{
				background_image:
					"Sử dụng khi bạn muốn có từ 2 Logo trở lên, chỗ nào muốn sử dụng logo thứ 2 này thì nhập class CSS <strong>web2-logo</strong> bao ngoài để phủ định class web-logo mặc định.",
				background_position:
					"Sử dụng khi bạn muốn logo dưới footer ở một vị trí khác so với top",
				background_size:
					"Sử dụng khi bạn muốn logo dưới footer có kích thước khác so với top",
				height:
					"Sử dụng khi bạn muốn logo dưới footer có Chiều cao khác so với top",
			},
			{
				height: "number",
			},
		...
		load_config_for_custom_logo(
			...
			{
				color:
					"Hỗ trợ dùng màu sắc riêng cho từng mục hoặc ô text đơn lẻ, , sử dụng class CSS: <strong>mcb2</strong>.",
			},
			{},
			data["cf_css_mcb2"],
			{
		...
		load_config_for_custom_logo(
			...
			{
				font_family:
					"Ý nghĩa tương tự <strong>fcb</strong>, sử dụng class CSS: <strong>fcb2</strong> mục đích hỗ trợ thêm fonts nếu bạn có nhu cầu.",
			},
			{},
			data["cf_css_fcb2"],
			{
		...
		load_config_for_custom_logo(
			...
			{
				font_family:
					"Khác với Font chữ (toàn trang), font này chỉ dùng cho 1 số module nhất định, để cho code trang trí và biến tấu website trên một số module nhất định (sử dụng class CSS: <strong>fcb</strong>). Mặc định chỉ hỗ trợ một font khác, trường hợp cần dùng thêm font, hãy sử dụng custom style để khai báo.",
			},
			{},
			data["cf_css_fcb"],
			{
...
//					'1366px' : 'HD 1366px',
			...
			{
				max_width:
					"Bạn có thể thiết lập chiều rộng tối đa cho khung, điều này giúp cho các khối HTML trong đó giữ được tính ổn định cao.",
			},
			{
				max_width: "number",
			},
		...
		load_config_for_custom_logo(
			...
			{
				width:
					"Fixed cứng chiều rộng khung w99. Mặc định là 999px, bạn có thể set thành auto hoặc một số cụ thể.",
				max_width:
					"Bạn có thể thiết lập chiều rộng tối đa cho khung, điều này giúp cho các khối HTML trong đó giữ được tính ổn định cao.",
			},
			{
...
//					'1600px' : 'HD+ 1600px',
			...
			},
			{
				...
				max_width:
					"Đây là kích thước lớn nhất của website mà bạn muốn đạt tới, thông thường thì ngưỡng phổ biến là HD 1366px, với các kích thước lớn hơn thì chi phí thiết kế cũng sẽ đắt đỏ hơn.",
				font_size:
					"Mặc định sẽ sử dụng cỡ chữ tiêu chuẩn là <strong>13px</strong> (~10pt), bạn có thể thay đổi kích thước này tại đây",
				font_family:
					'Font chữ mặc định là <strong>\'Helvetica Neue\', Helvetica, Arial, Tahoma, Verdana, sans-serif</strong>, trường hợp bạn muốn sử dụng font chữ khác, hãy nhập tại đây. <br><span class="orgcolor">* Lưu ý là các font chữ bạn chọn nếu không phải font phổ biến thì cần nhúng font vào website. Khuyên dùng: <a href="https://fonts.google.com/" rel="nofollow" target="_blank">https://fonts.google.com/</a></span>',
			},
			{
				max_width: "number",
				font_size: "number",
			...
			},
			{
				placeholder: {
					font_size: 13,
					font_family:
						'Helvetica, Arial, Tahoma, Verdana, -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Oxygen-Sans, Ubuntu, Cantarell, "Helvetica Neue", sans-serif',
				},
				/*
				'maxlength' : {
					'width' : 5,
		...
		load_config_for_custom_logo(
			...
			{
				background_color:
					"Màu nền tab nội dung trong trang chi tiết. Mã màu gồm dấu # và 3 hoặc 6 ký tự đại diện đằng sau.",
			},
			{
		...
		load_config_for_custom_logo(
			...
			{
				max_width:
					"Do thiết kế chiều rộng website càng lớn thì chi phí càng cao, nên mặc định nó sẽ được giới hạn lại, bạn muốn tăng nó lên hãy thay đổi tại đây.",
			},
			{
				max_width: "number",
			},
...
// config_theme
	...
	show_note_for_checkbox_config("cf_blog_hide_excerpt");
...
(function () {
	...
	let a = window.location.href.split("&tab=");
	if (a.length > 1) {
	...
	$(".eb-support-table tr").each(function () {
		...
		if ($("label", this).length == 0) {
			...
			if (
				$("td", this).length > 1 &&
				$("td:first input", this).length == 0 &&
				$("td:first textarea", this).length == 0 &&
				$("td:first select", this).length == 0
			) {
			...
			//
			if (get_input != "") {
				let a = $("td:first", this).html() || "";
				//				console.log('Auto create label for: ' + a);

				//
				if (a != "") {
					// lấy ID để label focus tới
					let label_for = $(get_input, this).attr("id") || "";
	...
	$(".eb-support-table label").click(function () {
		...
		//
		let a = $(this).attr("for") || $(this).attr("data-tab") || "";
	...
	setTimeout(function () {
		if (add_class_bg_for_tr_support == false) {
			...
			if (get_support_tab.length > 1 && $(".eb-support-table").length > 0) {
				...
				if (
					get_support_tab != "" &&
					$('label[for="' + get_support_tab + '"]').length > 0
				) {
					// cuộn chuột đến khu vực cần xem -> xem cho dễ
					window.scroll(
						0,
						$('label[for="' + get_support_tab + '"]').offset().top -
							$(window).height() / 3
					);
...
$(".click-to-set-site-color").click(function () {
	...
	let b =
		$('input[name="' + a + '"]').val() ||
		$('input[name="' + a + '"]').attr("placeholder") ||
		"";
	let n = prompt("Color code #:", b);
	//	console.log(n);
...
$(".click-to-reset-site-color").click(function () {
	...
	let b = $('input[name="' + a + '"]').attr("placeholder") || "";
	if (b != "") {
		$('input[name="' + a + '"]').val(b);
	}
});
	...
	if (a != "") {
		...
		for (let i = 0; i < a.length; i++) {
			...
			if (a[i] != "") {
				...
				str +=
					'<code class="d-block">{tmp.' + a[i].replace(/\-/g, "_") + "}</code>";
			}
		}
