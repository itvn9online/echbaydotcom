...
function WGR_order_details_after_update() {
	...
	if (top != self) {
		parent.WGR_order_list_after_update(
			order_id,
			document.frm_invoice_details.t_trangthai.value,
			jQuery(".bill-detail-status button.selected span").html()
		);
	}
}
...
function ___eb_admin_update_order_details() {
	...
	jQuery("#order_products").val(
		escape(JSON.stringify(arr_global_js_order_details))
	);

	//
	//	console.log( arr_global_js_order_customter );
	arr_global_js_order_customter["hd_ten"] = jQuery.trim(
		jQuery("#oi_hd_ten").val() || ""
	);
	arr_global_js_order_customter["hd_dienthoai"] = jQuery.trim(
		jQuery("#oi_hd_dienthoai").val() || ""
	);
	arr_global_js_order_customter["hd_diachi"] = jQuery.trim(
		jQuery("#oi_hd_diachi").val() || ""
	);

	// địa chỉ nâng cao
	arr_global_js_order_customter["hd_quanhuyen"] = jQuery.trim(
		jQuery("#oi_hd_quanhuyen").val() || ""
	);
	arr_global_js_order_customter["hd_id_quanhuyen"] = jQuery.trim(
		jQuery("#oi_hd_id_quanhuyen").val() || ""
	);
	arr_global_js_order_customter["hd_tinhthanh"] = jQuery.trim(
		jQuery("#oi_hd_tinhthanh").val() || ""
	);
	arr_global_js_order_customter["hd_id_tinhthanh"] = jQuery.trim(
		jQuery("#oi_hd_id_tinhthanh").val() || ""
	);

	//
	arr_global_js_order_customter["hd_chietkhau"] = jQuery.trim(
		jQuery("#hd_chietkhau").val() || ""
	);
	arr_global_js_order_customter["hd_phivanchuyen"] = jQuery.trim(
		jQuery("#hd_phivanchuyen").val() || ""
	);
	arr_global_js_order_customter["hd_admin_ghichu"] = jQuery.trim(
		jQuery("#hd_admin_ghichu").val() || ""
	);

	// tạo key để sau này tìm kiếm đơn hàng cho tiện
	arr_global_js_order_customter["hd_key"] = g_func.non_mark_seo(
		arr_global_js_order_customter["hd_ten"] +
			arr_global_js_order_customter["hd_dienthoai"] +
			arr_global_js_order_customter["hd_email"]
	);
	arr_global_js_order_customter["hd_key"] = arr_global_js_order_customter[
		"hd_key"
	].replace(/\-/g, "");

	//
	console.log(arr_global_js_order_customter);
	jQuery("#order_customer").val(
		escape(JSON.stringify(arr_global_js_order_customter))
	);
...
(function () {
	...
	if (
		...
		for (var i = 0; i < arr.length; i++) {
			...
			if (typeof arr[i].__eb_hd_customer_info != "undefined") {
				...
				for (var x in arr2) {
					order_details_arr_cart_customer_info[x.replace("__eb_", "")] =
						arr2[x];
				}
		...
		setTimeout(function () {
			...
			//
			if (
				jQuery("#order_products").val().length > 100 &&
				jQuery("#order_customer").val().length > 100
			) {
	...
	for (var i = 0; i < arr.length; i++) {
		...
		if (arr[i].color != "") {
			...
			str_edit_size_color +=
				'<span title="Đổi màu mới" data-id="' +
				arr[i].id +
				'" class="cur click-edit-order-color bluecolor"><strong> - Color: </strong>' +
				arr[i].color +
		...
		if (arr[i].size != "") {
			...
			str_edit_size_color +=
				' <span title="Đổi size mới" data-id="' +
				arr[i].id +
				'" class="cur click-edit-order-size bluecolor">' +
				arr[i].size +
				' <i class="fa fa-edit"></i></span>';
		}
		...
		<tr>\
			...
			<td><input type="number" value="' +
				...
				'" data-id="' +
				arr[i].id +
				'" class="change-update-cart-quantity ss" size="5" maxlength="10" /></td>\
			<td><span class="ebe-currency">' +
				g_func.money_format(total_line) +
				"</span></td>\
		</tr>"
		);
	}
	...
	jQuery(".change-update-cart-quantity").change(function () {
		...
		for (var i = 0; i < arr_global_js_order_details.length; i++) {
			if (
				typeof arr_global_js_order_details[i].id != "undefined" &&
	...
	jQuery(".click-edit-order-color").click(function () {
		...
		if (a != null && a != v) {
			jQuery(this).html(
				"<strong> - Color: </strong>" + a + ' <i class="fa fa-edit"></i>'
			);
	...
	jQuery(".click-edit-order-size").click(function () {
		...
		if (a != null && a != v) {
			jQuery(this).html(
				" (<strong>Size: </strong>" + a + ') <i class="fa fa-edit"></i>'
			);
	...
	//
	jQuery("#hd_chietkhau, #hd_phivanchuyen")
		.change(function () {
			WGR_admin_tinh_so_luong_hoa_don();
			co_thay_doi_chua_duoc_luu = true;
	...
	try {
		...
		if (
			typeof cus["hd_chietkhau"] != "undefined" &&
			cus["hd_chietkhau"] != ""
		) {
		...
		if (
			typeof cus["hd_discount_code"] != "undefined" &&
			cus["hd_discount_code"] != ""
		) {
			...
			jQuery
				.ajax({
					...
					type: "GET",
					url: ajaxl_url(
						"check_discount_code&code=" + cus["hd_discount_code"] + "&no_echo=1"
					),
					data: "",
				})
				.done(function (msg) {
					if (typeof msg == "object") {
						...
						if (typeof msg["error"] != "undefined") {
							...
							jQuery("#oi_ma_giam_gia").append(
								' <span class="redcolor">' + msg["msg"] + "</span>"
							);
						} else {
							var m2 = "";
							...
							if (
								...
								jQuery(".each-for-set-cart-value").each(function () {
									...
									if (
										cart_pid > 0 &&
										cart_pid * 1 == msg["coupon_product"] * 1
									) {
							...
							if (m2 != "") {
								$("#oi_ma_giam_gia").append(
									' - Lưu ý: <span class="small greencolor">' + m2 + "</span>"
								);
							}
						...
						console.log(
							"%c Lỗi cú pháp! vui lòng báo cho kỹ thuật viên. Xin cảm ơn",
							"color: red;"
						);
					}
				});
		...
		if (
			typeof cus["hd_phivanchuyen"] != "undefined" &&
			cus["hd_phivanchuyen"] != ""
		) {
	...
	for (var x in arr_dich) {
		if (typeof cus[x] != "undefined") {
			...
			if (x == "hd_usertime") {
				...
				if (cus[x].toString().includes("1970") == true) {
...
setTimeout(function () {
	...
	jQuery(
		'.eb-order-filter-tab li[data-tab="' +
			document.frm_invoice_details.t_trangthai.value +
			'"] a'
	).addClass("selected");
	...
	if (
		WGR_check_option_on(order_user_can) == false &&
		e != "" &&
		e.split("@").length == 2 &&
		e.includes(document.domain) == false
	) {
		if (e.length > 3) {
			uri += "&e=" + e;
		}
	...
	if (uri != "") {
		ajaxl(
			"get_customer_blacklist&no_echo=1&order_id=" + order_id + uri,
			"oi_customer_blacklist",
			1,
			function () {
				setTimeout(function () {
					...
					if (arr_user_blacklist.length > 0) {
						if (
							arr_user_blacklist.length == 1 &&
							typeof arr_user_blacklist[0].error != "undefined"
						) {
							console.log(arr_user_blacklist[0].error);
						} else {
							// hiển thị thông báo báo xấu
							jQuery("#open_list_bao_xau strong").html(
								arr_user_blacklist.length
							);
							...
							jQuery("#open_list_bao_xau")
								...
								.on("click", function () {
									...
									for (var i = 0; i < arr_user_blacklist.length; i++) {
										...
										try {
											var custom_info = jQuery.parseJSON(
												unescape(arr_user_blacklist[i].order_customer)
											);
											console.log(custom_info);
										...
										<a href="' +
												admin_link +
												"admin.php?page=eb-order&id=" +
												arr_user_blacklist[i].order_id +
												'" target="_blank" class="bold">' +
												custom_info.hd_ten +
												'</a>\
										<ul class="sub-menu">\
											<li>Ghi chú: <span class="orgcolor">' +
												custom_info.hd_admin_ghichu +
												"</span></li>\
											<li>Điện thoại: " +
												custom_info.hd_dienthoai +
												"</li>\
											<li>Email: " +
												custom_info.hd_email +
												"</li>\
											<li>Địa chỉ: " +
												custom_info.hd_diachi +
												"</li>\
											<li>Ngày gửi: " +
												_date(lang_date_time_format, custom_info.hd_usertime) +
												'</li>\
											<li class="d-none">Thiết bị: ' +
												custom_info.hd_agent +
												"</li>\
										</ul>\
									</li>";
										} catch (e) {
					...
					if (arr_list_orther_order.length > 0) {
						...
						for (var i = 0; i < arr_list_orther_order.length; i++) {
							try {
								var custom_info = jQuery.parseJSON(
									unescape(arr_list_orther_order[i].order_customer)
								);
								//							console.log( custom_info );
								...
								str +=
									'\
							<tr class="hd_status' +
									arr_list_orther_order[i].order_status +
									'">\
								<td><a href="' +
									admin_link +
									"admin.php?page=eb-order&id=" +
									arr_list_orther_order[i].order_id +
									'" target="_blank" class="bold">' +
									arr_list_orther_order[i].order_id +
									'</a></td>\
								<td><a href="' +
								...
								<td><a href="' +
									admin_link +
									"admin.php?page=eb-order&by_ip=" +
									arr_list_orther_order[i].order_ip +
									'" target="_blank">' +
									arr_list_orther_order[i].order_ip +
									"</a></td>\
								<td>" +
...
(function () {
	if (jQuery(".dang-xac-nhan").length > 0) {
		...
		if (a != "" && a * 1 != mtv_id) {
			...
			jQuery('form[name="frm_invoice_details"]')
				.css({
					opacity: 0.1,
				})
				.animate(
...
jQuery("#eb_vandon_print").click(function () {
	window.open(
		web_link + "billing_print?order_id=" + order_id + "&f=print_van_don",
		"_blank"
	);
});
	...
	.focus(function () {
		...
		if (dog("ui_hd_quanhuyen") == null) {
			...
			(function () {
				...
				//
				jQuery("#oi_hd_quanhuyen").after(
					'<div id="ui_hd_quanhuyen"><ul>' + str + "</ul></div>"
				);
			})();
...
function WGR_order_details_add_city() {
	jQuery("#oi_hd_quanhuyen").val(
		jQuery("#ui_hd_quanhuyen li.orgcolor").attr("data-dist-name") || ""
	);
	jQuery("#oi_hd_id_quanhuyen").val(
		jQuery("#ui_hd_quanhuyen li.orgcolor").attr("data-dist") || ""
	);

	jQuery("#oi_hd_tinhthanh").val(
		jQuery("#ui_hd_quanhuyen li.orgcolor").attr("data-city-name") || ""
	);
	jQuery("#oi_hd_id_tinhthanh").val(
		jQuery("#ui_hd_quanhuyen li.orgcolor").attr("data-city") || ""
	);
...
function WGR_auto_add_city_with_address() {
	...
	if (
		jQuery.trim(jQuery("#oi_hd_quanhuyen").val()) == "" &&
		jQuery.trim(jQuery("#oi_hd_tinhthanh").val()) == ""
	) {
