...
function WGR_add_img_to_menu(img) {
	if (img.includes("wp-content/uploads/") == true) {
		img = "wp-content/uploads/" + img.split("wp-content/uploads/")[1];
	}

	WGR_event_add_img_edit_menu(
		add_media_for_menu,
		"img",
		img.replace(web_link, "")
	);
}
...
function WGR_event_add_img_edit_menu(jd, tai, add_lnk) {
		...
		if (tai == "img") {
			...
			if (gallery_has_been_load == false) {
				...
				ajaxl("gallery", "oi_admin_popup", 9, function () {
					...
					jQuery(
						"#oi_admin_popup .eb-newgallery-option .gallery-add-to-menu-edit"
					).show();
	...
	// add
	if (tai == "icon") {
		if (lnk.includes("fa-") == false) {
			lnk = "fa-" + lnk;
		}
		if (lnk.includes("fa fa-") == false) {
			lnk = "fa fa-" + lnk;
		}
...
function WGR_add_img_edit_menu() {
	jQuery(".menu-item-edit-active").each(function () {
		if (jQuery(".wgr-button-menu", this).length == 0) {
			...
			//
			jQuery(".edit-menu-item-title", this).after(
				'<div class="wgr-button-menu">\
				<button type="button" onclick="WGR_event_add_img_edit_menu(\'' +
					a +
					'\', \'img\')" data-add="img" data-id="' +
					a +
					'" class="button wgr-button-add-img">Add image</button>\
				<button type="button" onclick="WGR_event_add_img_edit_menu(\'' +
					a +
					'\', \'icon\')" class="button wgr-button-add-icon">Add icon (font awesome)</button>\
			</div>\
...
function WGR_main_edit_menu() {
	...
	//
	jQuery("#side-sortables ul.outer-border").after(
		jQuery("#content-for-quick-add-menu").html() || ""
	);
	...
	jQuery("#nav-menus-frame").before(
		'<br><div class="quick-add-menu-to-admin-menu"><input type="text" id="wgr_search_product_in_menu" placeholder="Tìm kiếm Sản phẩm/ Bài viết... để thêm vào menu" class="wgr-search-post-menu" /></div>'
	);
	...
	jQuery("#wgr_search_product_in_menu").click(function () {
		if (dog("show_all_list_post_page_menu") == null) {
            ...
            */
			...
			// nội dung tìm kiếm
			jQuery("#wgr_search_product_in_menu").after(
				'<p class="orgcolor">* Nhập từ khóa vào ô tìm kiếm để tìm kiếm Sản phẩm, bài viết tin tức, trang tĩnh, chuyên mục, danh mục... sau đó bấm chọn trong danh sách vừa tim được để thêm vào menu.</p><div id="show_all_list_post_page_menu"><ul></ul></div>'
			);

			//
			WGR_load_post_page_for_add_menu(
				eb_site_group,
				"category",
				"Chuyên mục sản phẩm",
				"taxonomy"
		...
		if ($(window).width() > 1100) {
			$("#show_all_list_post_page_menu").height(
				$(window).height() -
					$("#show_all_list_post_page_menu").offset().top -
					25
			);
		}
	});
