...
(function () {
    ...
    var eb_this_current_url = jQuery('link[rel="canonical"]:first').attr('href') || window.location.href.split('?')[0].split('&')[0].split('#')[0];
...
function ___eb_set_url_for_search_price_in_button(clat) {
    ...
    } else if (switch_taxonomy == 'category' || switch_taxonomy == 'post_options') {} else {
        if (WGR_check_option_on(cf_tester_mode)) {
            console.log('search price is active, but run only category page -> STOP.');
        }
    }
...
function ___eb_set_url_for_search_advanced_button(clat, inner_clat, go_to_url) {
	...
	*/
    ...
    if (jQuery(clat + ' a.click-to-search-advanced').length == 0) {
        ...
        jQuery(inner_clat).append('<div class="global-button-for-seach-advanced"><a href="javascript:;" class="click-to-search-advanced search-advanced-btn d-none whitecolor"><i class="fas fa-search"></i> <span>Lọc sản phẩm</span></a></div>');
    }
