...
//
function load_slider_for_home(i) {
	if (typeof i != 'number') {
		i = 50;
	}

	//
	if (typeof jEBE_multi_slider == 'undefined') {
		if (i > 0) {
			setTimeout(function () {
				load_slider_for_home(i - 1);
			}, 200);
		}
		return false;
	}

	// chạy vòng lặp và tạo các ID ngẫu nhiên để kích hoạt slider
	var list_run_slider = [];
	var ops_run_slider = {
		autoplay: false,
		swipemobile: false,
	...
	//_global_js_eb.auto_margin();
	$('.home-blogs-slider .widget_random_blog .widget-run-slider').each(function () {
		var slider_id = '_' + Math.random().toString(32).replace(/\./, '_');
		//console.log(slider_id);
		$(this).attr({
			'id': slider_id
		});

		// config của slider thì tạo widget Echbay run slider rồi copy config của nó
		list_run_slider.push('#' + slider_id);
	});

	$(document).ready(function () {
		jEBE_multi_slider(list_run_slider.join(','), ops_run_slider, function () {
			console.log(list_run_slider);
			
			// chỉnh lại chiều cao của slider
			for ( var i = 0; i < list_run_slider.length; i++ ) {
				var slier_height = $(list_run_slider[i] + ' li:first').height();
				
				//
				$(list_run_slider[i]).css({
					'height': slier_height + 'px',
					'line-height': slier_height + 'px'
				});
			}
		});
	});
