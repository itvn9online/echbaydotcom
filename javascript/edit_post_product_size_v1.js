...
function WGR_run_for_admin_edit_ads_post(action_id) {
	...
	$("#" + action_id)
			...
			if (dog(jd_for_quick_search_post) == null) {
				...
				if (action_id == "_eb_ads_for_post") {
					if (eb_posts_list.length > 0) {
						$("#" + jd_for_quick_search_post).append(
		...
		.blur(function () {
			...
			//
			if (
				$("#_eb_ads_for_post").val() == "" &&
				$("#_eb_ads_for_category").val() == ""
			) {
...
function eb_func_show_product_size() {
	...
	$("#" + eb_inner_html_product_size).html(
		str_size +
			'<div class="small">Chức năng dùng để tạo danh sách Kích thước, dung tích... cho sản phẩm và mức giá riêng (nếu có).</div>'
	);
	...
	$("#" + eb_inner_html_product_size + " li")
		...
		.click(function () {
			...
			if (a == "") {
				...
				//
				$(".eb-input-edit-product-size button")
					.off("click")
					.click(function () {
				...
				*/
				...
				// thêm size
					...
					setTimeout(function () {
						$('.eb-admin-product-size li[data-add="1"]')
							.prev()
							.addClass("redcolor")
							.addClass("selected")
							.click();
...
function WGR_run_for_admin_edit_post() {
	...
	*/
	...
	(function () {
		...
		$("#_eb_product_oldprice, #_eb_product_price")
			.change(function () {
				var a = $(this).val() || 0;
	...
	setTimeout(function () {
		...
		//
		$(".eb-input-edit-product-color button")
			.off("click")
			.click(function () {
				var a = $(this).attr("data-action") || "";
