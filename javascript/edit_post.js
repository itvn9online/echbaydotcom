...
function WGR_post_edit_change_url_avt(a) {
	...
	<tr data-row="_eb_show_product_avatar">\
		<td class="t">&nbsp;</td>\
		<td class="i"><div class="show_product_avatar" style="background-image:url(\'' +
			a +
			"')\">&nbsp;</div></td>\
	</tr>"
	);
...
function WGR_add_value_form_edit_size(arr) {
	...
	jQuery("#eb_input_edit_product_size_price").val(
		g_func.money_format(arr.price)
	);

	//
	jQuery(".eb-input-edit-product-size")
		.css({
		...
		.click(function () {
			...
			if (jQuery("#eb_input_edit_color_size_node").val() != "") {
				...
				if (a == "save") {
					admin_global_size_color[a_node] = {
						...
						price: jQuery("#eb_input_edit_product_size_price")
							.val()
							.replace(/\,/g, ""),
					};
				} else if (a == "remove") {
					admin_global_size_color[a_node] = null;
					admin_global_size_color = WGR_remove_add_null_value(
						admin_global_size_color
					);
				}
			...
			// size chính
				...
				if (a == "save") {
					...
					eb_global_product_size = WGR_remove_add_null_value(
						eb_global_product_size
					);
...
function WGR_show_list_size_in_color() {
	...
	str +=
		'<li data-add="1" title="Thêm size (color) mới"><i class="fas fa-plus"></i></li>';

	jQuery(".eb-admin-product-color-size").html(str);

	// hiệu ứng chỉnh size
	jQuery(".eb-admin-product-color-size li")
		.off("click")
		.click(function () {
	...
	if (admin_global_size_color.length > 0) {
		jQuery("#eb_input_edit_product_size_bycolor_id").val(
			escape(JSON.stringify(admin_global_size_color))
		);
	} else {
		jQuery("#eb_input_edit_product_size_bycolor_id").val("");
	}
...
function WGR_ads_get_current_select_category_or_post_name(action_id) {
	...
	if (a != "") {
		...
		if (action_id == "_eb_ads_for_post") {
			if (
				WGR_ads_show_current_select_category_or_post_name(
					a,
					eb_posts_list,
					action_id
				) == false
...
function WGR_run_for_admin_edit_ads_post(action_id) {
	...
	jQuery("#" + action_id).after(
		'<div><em class="small bluecolor show-for-' + action_id + '"></em></div>'
	);
	...
	jQuery("#" + action_id)
		...
		.click(function () {
			...
			if (dog(jd_for_quick_search_post) == null) {
				jQuery(this).after(
					'<div class="admin-show-quick-search"><div id="' +
						jd_for_quick_search_post +
						'" class="ads-show-quick-search-post"></div></div>'
				);

				//
				if (action_id == "_eb_ads_for_post") {
					if (eb_posts_list.length > 0) {
						jQuery("#" + jd_for_quick_search_post).append(
							edit_post_load_list_post_for_quick_search(
								eb_posts_list,
								"Sản phẩm (Post)"
							)
						);
					...
					if (eb_site_group.length > 0) {
						jQuery("#" + jd_for_quick_search_post).append(
							edit_post_load_list_taxonomy_for_quick_search(
								eb_site_group,
								"Chuyên mục sản phẩm"
							)
						);
		...
		.blur(function () {
			...
			//
			if (
				jQuery("#_eb_ads_for_post").val() == "" &&
				jQuery("#_eb_ads_for_category").val() == ""
			) {
...
function edit_post_load_list_post_for_quick_search(
	arr,
	arr_name,
	permarklink,
	taxonomy
	...
	return (
		...
		arr_name +
		"</h4><ul>" +
		edit_post_load_list_details_post_for_quick_search(
			arr,
			permarklink,
			taxonomy
		) +
...
function edit_post_load_list_details_post_for_quick_search(
	arr,
	permarklink,
	taxonomy,
	child_of
	...
	for (let i = 0; i < arr.length; i++) {
		...
		if (typeof arr[i].arr == "object" && arr[i].arr.length > 0) {
			str += edit_post_load_list_details_post_for_quick_search(
				arr[i].arr,
				permarklink,
				taxonomy,
				child_of + "- "
...
function edit_post_load_list_taxonomy_for_quick_search(
	arr,
	arr_name,
	taxonomy
) {
...
function eb_func_add_nut_product_size(str, i) {
	...
	<div class="eb-admin-product-size">\
		...
		'\
			<li data-parent="' +
		i +
		'" data-add="1" title="Thêm size mới"><i class="fas fa-plus"></i></li>\
		</ul>\
	</div>'
	);
}
...
function eb_func_show_product_size() {
	...
	if (typeof eb_global_product_size != "object") {
		...
		*/
		...
		if (
			eb_global_product_size.length > 0 &&
			...
			for (let i = 0; i < eb_global_product_size.length; i++) {
				...
				if (
					typeof eb_global_product_size[i].soluong != "undefined" &&
...
//				console.log( JSON.stringify( eb_global_product_size ) );
	...
	jQuery("#" + eb_inner_html_product_size).html(
		str_size +
			'<div class="small">Chức năng dùng để tạo danh sách Kích thước, dung tích... cho sản phẩm và mức giá riêng (nếu có). Nếu phần giá riêng này được thiết lập, nó sẽ được sử dụng để thay thế giá mặc định của sản phẩm.</div>'
	);
...
function eb_func_click_modife_product_size() {
	jQuery("#" + eb_inner_html_product_size + " li")
		.off("click")
		.click(function () {
			//		console.log(1);
...
//					jQuery('.eb-admin-product-size li[data-add="1"]').prev().addClass('redcolor').addClass('selected').click();
				...
				*/
					jQuery(
						'.eb-admin-product-size li[data-node="' +
							(eb_global_product_size.length - 1) +
							'"]'
					).click();
				}
			}
...
function WGR_run_for_admin_edit_post() {
	...
	*/
	...
	(function () {
		...
		jQuery("#_eb_product_avatar").change(function () {
			...
			// có thì add thêm vào
				...
				if (jQuery('tr[data-row="_eb_show_product_avatar"]').length > 0) {
					jQuery(
						'tr[data-row="_eb_show_product_avatar"] .show_product_avatar'
					).css({
			...
			if (img == "") {
				if (
					...
					if (a != "") {
						...
						$("#_eb_product_avatar")
							.val("http://i3.ytimg.com/vi/" + a + "/hqdefault.jpg")
							.change();
					}
				}
			}
		...
		*/
		...
		<div class="ebe-fixed-content-style">\
			...
			<label for="click_replace_content">Thay thế nội dung trong bài viết theo <a href="' +
				web_link +
				'wp-admin/admin.php?page=eb-config&tab=cache&support_tab=cf_replace_content_full" target="_blank">cấu hình website</a> (nếu có).</label>\
		</div>\
		<div class="ebe-fixed-content-style">\
						...
						if (current_id == "") {
							...
							if (current_id != "") {
								current_id =
									...
									current_id +
									"_" +
									Math.random(32).toString().replace(".", "_");
								jQuery(this).attr({
									id: current_id,
								});
							}
			...
			if (iff_id == "_eb_product_list_color_ifr") {
				...
				jQuery("#" + iff_id)
					...
					.click(function () {
						...
						f.eb_input_edit_product_color_price.value =
							g_func.money_format(price);
	...
	// v4
	jQuery("#publish")
		.addClass("publish-position-fixed")
		/*
	.css({
		'position' : 'fixed',
		'bottom': '20px',
	...
	*/
	...
	jQuery(window).on("load", function () {
		if (typeof EBE_set_default_title_for_seo == "function")
			EBE_set_default_title_for_seo();
	});
	...
	jQuery("body").append(
		"<style>div.gallery-add-to-post_avt { display: block; }</style>"
	);
...
function WGR_show_name_for_truong_tuy_bien() {
	...
	for (let i = 0; i < arr_cf_truong_tuy_bien.length; i++) {
		...
		if (arr_cf_truong_tuy_bien[i] != "") {
			...
			if ($('#metakeyselect option[value="' + a + '"]').length > 0) {
				$('#metakeyselect option[value="' + a + '"]').html(
					arr_cf_truong_tuy_bien[i]
				);
			} else {
				str +=
					'<option value="' +
	...
	$("#the-list tr").each(function () {
		...
		if (this_id != "" && $('input[id="' + this_id + '-key"]').length > 0) {
			...
			if (
				...
				$("tr#" + this_id + " td.left").before(
					'<td class="the-list-td-left bold">' +
						arr_ten_truong_tuy_bien[td_key] +
						"</td>"
				);
			} else {
				$("tr#" + this_id + " td.left").before(
					'<td class="the-list-td-left bold">&nbsp;</td>'
				);
			}
			colspan_th = 1;
		}
	...
	if (colspan_th == 1) {
		...
		$("#list-table tr th.left")
			.after('<th class="the-list-th-center">Key</th>')
			.addClass("the-list-th-left");
		$("#list-table tr th:last").addClass("the-list-th-right");

		$("#the-list td.left").addClass("the-list-td-center");
		$("#the-list td.the-list-td-center")
			.dblclick(function () {
				$(this).addClass("hide-this-before");
			})
			.attr({
				title:
					"Nhấp đúp chuột để mở chức năng sửa KEY (chỉ sửa nếu thực sự cần thiết)",
			});
