	...
	jQuery( content_id ).contents().find( 'img' ).removeClass( class_for_download_img_to_site ).each(function() {
		...
		if ( a != '' ) {
			...
			if ( current_domain != b ) {
				...
				download_url = web_link + 'download_img_to_site/?img=' + encodeURIComponent( a ) + '&for_id_content=' + encodeURIComponent( content_id ) + '&post_ID=' + ( $('#post_ID').val() || '' ) + '&img_title=' + encodeURIComponent( jQuery(this).attr('alt') || '' );
