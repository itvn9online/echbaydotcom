    ...
    jQuery(content_id).contents().find('img').removeClass(class_for_download_img_to_site).each(function () {
        ...
        if (a != '') {
            ...
            if (current_domain != b) {
                ...
                download_url = web_link + 'download_img_to_site/?img=' + encodeURIComponent(a) + '&for_id_content=' + encodeURIComponent(content_id) + '&post_ID=' + (jQuery('#post_ID').val() || '') + '&img_title=' + encodeURIComponent(jQuery(this).attr('alt') || '');
...
function WGR_load_GA_for_admin_tracking() {
    ...
    if (adminpage == 'toplevel_page_eb-order') {} else {
...
function WGR_show_widget_name_by_title() {
    ...
    //
    jQuery('.eb-get-widget-title').off('change').change(function () {
    ...
    if (typeof isLogin != 'undefined' && isLogin == 1) {
        jQuery('.widget').each(function () {
            if (jQuery('.disable-click-edit-widget', this).length > 0) {
                ...
                if (tit != '') {
                    jQuery('.in-widget-title', this).removeClass('in-widget-title').addClass('small bluecolor').html(' * ' + tit);
                }
            }
        });
    }
    // những người khác thì tắt luôn đi
        ...
        jQuery('#widgets-right .widget').each(function () {
            ...
            if (jQuery('.disable-click-edit-widget', this).length > 0) {
                jQuery('.in-widget-title', this).removeClass('in-widget-title').addClass('small redcolor').html(' * Dành riêng cho kỹ thuật, không tự ý sửa!');

                jQuery('.widget-title, .widget-title-action', this).off('click').click(function () {
...
function WGR_widget_add_custom_style_to_field(field_name) {
    ...
    jQuery('.click_add_widget_class[data-add="' + field_name + '"]').off('click').click(function () {
        ...
        if (a != '') {
            ...
            jQuery('input[name="' + field_name + '"]').val(jQuery.trim(c)).change();
